...
class VeinGenerator(Dataset):
    ...
    def extract_locations(self, masks, rois=None):
        ...
        for i in range(len(masks)):
            ...
            if rois is not None:
                ...
                if self.dilate > 0:
                    roi = ndimage.binary_dilation(
                        input=roi.astype(bool), iterations=self.dilate
                    ).astype(float)
            ...
            idx = np.concatenate(
                [i * np.ones_like(idx[:, 0:1]), idx], axis=1
            )  # [(N+1)*F, 3]
            indices.append(idx)
    ...
    def __getitem__(self, index):
        ...
        locs = (
            np.array([[j, k]]) + np.round(np.dot(rotation, self.image_slice.T).T)
        ).astype(int)
        ...
        if self.augment:
            ...
            tile = torchvision.transforms.GaussianBlur(
                kernel_size=kernel, sigma=(0.1, 1)
            )(tile)
