...
class LeafGenerator(Dataset):
    ...
    def extract_locations(self, masks):
        ...
        for i in mask_range:
            ...
            contour = np.concatenate(
                [i * np.ones_like(contour[:, 0:1]), contour], axis=1
            )  # [N, 3]
    ...
    def contour2channel(self, contour, mask):
        ...
        contour_channel[
            np.round(contour[:, 1]).astype(int), np.round(contour[:, 0]).astype(int)
        ] = 1
    ...
    def __getitem__(self, index, debug=False):
        ...
        old_idx = np.round(np.linspace(0, len(old_trace) - 1, self.path_length)).astype(
            int
        )
        old_trace = old_trace[old_idx]
        new_idx = np.round(np.linspace(0, len(new_trace) - 1, self.path_length)).astype(
            int
        )
