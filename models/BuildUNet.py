    ...
    def __init__(
        ...
        for i, (in_channels, out_channels) in enumerate(
            ...
            for j in range(num_convs):
                if verbose:
                    print(f"encoder conv {i}-{j}: {in_channels} -> {out_channels}")
            ...
            if i < len(layers) - 1:
                ...
                if verbose:
                    print(f"encoder pool {i}  : {out_channels}")
                    print()
        if verbose:
            print()
        ...
        for i, (in_channels, out_channels) in enumerate(
            zip(decoder_layers[:-1], decoder_layers[1:])
        ):
            ...
            if verbose:
                print(
                    f"decoder up   {i}  : {in_channels} -> {out_channels} + {encoder_layers[-i - 2]}"
                )
            for j in range(num_convs):
                in_channels = (
                    out_channels + encoder_layers[-i - 2] if j == 0 else out_channels
                )
                if verbose:
                    print(f"decoder conv {i}-{j}: {in_channels} -> {out_channels}")
        ...
        if verbose:
            print(f"output conv: {decoder_layers[-1]} -> {output_channels}")
            print()
    ...
    def forward(self, x):
        ...
        for i in range(len(self.layers)):
            for j in range(self.num_convs):
                ...
                if self.verbose:
                    print(f"encoder conv {i}-{j}: {x.shape}")
            x_cache.append(x)
            if i < len(self.layers) - 1:
                x = self.encoder_pools[i](x)
                if self.verbose:
                    print(f"encoder pool {i}  : {x.shape}")
                    print()
        if self.verbose:
            print()
        ...
        for i in range(len(self.layers) - 1):
            ...
            if self.verbose:
                print(f"decoder up   {i}  : {x.shape} + {x_cache[-i - 2].shape}")
            ...
            for j in range(self.num_convs):
                ...
                if self.verbose:
                    print(f"decoder conv {i}-{j}: {x.shape}")
            if self.verbose:
                print()
