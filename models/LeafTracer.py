...
class LeafTracer:
    ...
    def trace(self, image, tile, idx, max_iter=500, warmup_iter=10):
        ...
        for iter in range(max_iter):
            ...
            pred_cols, pred_rows = (
                pred[0, : self.step_length],
                pred[1, : self.step_length],
            )
            ...
            if iter > 2 * warmup_iter:
                ...
                dists = (
                    (global_cols - cols[index]) ** 2 + (global_rows - rows[index]) ** 2
                ) ** 0.5
            ...
            previous_rows = (rows[n:] - row + self.window_size / 2).clip(
                0, self.window_size - 1
            )
            previous_cols = (cols[n:] - col + self.window_size / 2).clip(
                0, self.window_size - 1
            )
